...
const addMonthsToYmd = (ymd, monthsToAdd) => {
  ...
  let month = parsed.month + monthsToAdd;
  ...
  const day = Math.min(parsed.day, dim);
...
const getItemUnitPrice = (item) => {
  return Number(item?.selectedPlan?.price || 0);
};

const getProductDeposit = (product) => {
  return Number(product?.deposit || 0);
};
...
function CheckoutPage() {
  ...
  // Calculations
  const { rentTotal, depositTotal, insuranceAmount, grandTotal, orderItems } =
    useMemo(() => {
      const validItems = cartItems.filter((item) => item?.productId);

      const rent = validItems.reduce(
        (sum, item) => sum + getItemUnitPrice(item) * item.quantity,
        0
      );

      const deposit = validItems.reduce(
        (sum, item) => sum + getProductDeposit(item.productId) * item.quantity,
        0
      );

      const insurance = isInsuranceSelected ? rent * 0.1 : 0;

      const total =
        rent + deposit + transport + platformCharge + insurance;
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      await axios.post(`${API_BASE_URL}/api/orders/create`, {
        ...
        grandTotal,
        deliveryDate,
        returnDate: addMonthsToYmd(deliveryDate, 1),
      });
